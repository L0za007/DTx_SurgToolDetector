import random, cv2
import numpy as np
from data.data_aug.bbox_util import *

def _there_are_boxes(boxes):
    ...
    def __call__(self, img, bboxes):
        ...
        if random.random() < self.p:
            ...
            if _there_are_boxes(bboxes):
                ...
                bboxes[:, 2] += box_w
            #else:
            #    print("No box in ", self.__class__.__name__,bboxes)
...
class RandomShear(object):
    ...
    def __init__(self, shear_factor = 0.2):
        ...
        if type(self.shear_factor) == tuple:
            assert len(self.shear_factor) == 2, "Invalid range format, it should be (min,max)"   
        else:
    ...
    def __str__(self) -> str:
        ...
        for i, aug in enumerate(self.augmentations):
            if type(self.probs) == list:
                ...
                prob = self.probs
            string += f"T{i}:{str(aug)}, prob={prob}\n"
        return string

TRANSF_DICT = {'rotate': RandomRotate, 'flip_horizontal': RandomHorizontalFlip, 'scale': RandomScale,
               'translate': RandomTranslate,        'shear': RandomShear,            'hsv': RandomHSV  }